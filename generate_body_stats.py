...
if __name__ == '__main__':
    ...
    for key in bodies_annot_data:
        if key.isdigit():
            get_body_annotation = body_annotations_name + "/key/" + key
            response_body_annot = node_service.custom_request( get_body_annotation, "", ConnectionMethod.GET )
            body_annot_data = json.loads(response_body_annot)
            #print "key " + key
            # key is a bodyID
            # get synapses for bodyID like this http://emdata1.int.janelia.org:8500/api/node/44d42/mb6_synapses/label/10095139
            get_synapses_dvid = annotations_synapses + "/label/" + key + "?relationships=false"
            ...
            if response_syn == 'null':
                ...
                if mid != 0:
                    ...
                    if 'status' in body_annot_data:
                        export_synapse_data["status"] = body_annot_data["status"]
                    if 'name' in body_annot_data:
                        export_synapse_data["name"] = body_annot_data["name"]
